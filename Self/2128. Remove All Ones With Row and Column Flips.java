...
// Review 23/3/10 - have no clue, needs help from above solutions
// M1: time O(mn) space O(1) with grid changed
class Solution {
    public boolean removeOnes(int[][] grid) {
        for (int j = 0; j < grid[0].length; j++) {
            if (grid[0][j] != 0) {
...
// M2: time O(mn) space O(1) without grid changed
class Solution {
    public boolean removeOnes(int[][] grid) {
        for (int i = 1; i < grid.length; i++) {
            for (int j = 0; j < grid[0].length; j++) {
                if (Math.abs(grid[i][j] - grid[0][j]) != Math.abs(grid[i][0]-grid[0][0])) {return false;}
            }
        }
        return true;
    }
